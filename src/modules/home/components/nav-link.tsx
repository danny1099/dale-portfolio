...
export const Link = ({ children, route, icon, color, child, className }: Props) => {
  ...
  return (
    <Navlink href={route} scroll variant="link" className={cn('px-3 h-10 w-full hover:bg-secondary hover:text-tertiary',pathname.includes(route) && 'bg-secondary text-tertiary hover:bg-secondary hover:text-tertiary', className)}>
      {child && child}
